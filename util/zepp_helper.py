...
def get_current_step(app_token, userid):
    """读取 Zepp Life 云端当天已同步的步数"""
    today = get_beijing_time().strftime("%F")
    ...
    try:
        ...
        for query_type in ("detail", "summary"):
            ...
            if isinstance(band_data, list):
                ...
                for date_str, day_data in band_data.items():
                    if isinstance(day_data, dict):
                        entries.append({"date": date_str, "summary": day_data})

            for item in entries:
                if not isinstance(item, dict):
                    continue
                date_str = str(item.get("date_time") or item.get("date") or "")
                if date_str and not date_str.startswith(today):
                    continue
                ...
                if isinstance(summary, str):
                    try:
                        ...
                        summary = json.loads(base64.b64decode(summary).decode("utf-8"))
                if not isinstance(summary, dict):
                    continue
                current_step = summary.get("stp", {}).get("ttl")
                if current_step is not None:
                    current_steps.append(int(current_step))
            if current_steps:
